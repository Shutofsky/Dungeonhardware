...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  int pos1=0, pos2=0;
  String msgAddr="";
  pongAns = myIPAddr + "/PONG";
  Serial1.print("PONG Answer: ");
  Serial1.println(pongAns);
  ...
  cmd_buffer[length] = 0; 
  Serial1.println(myIPAddr);
  Serial1.println(cmd_buffer);
  pos1 = cmd_buffer.indexOf('/',0);
  msgAddr = cmd_buffer.substring(0,pos1);
  if (msgAddr.compareTo(myIPAddr)==0 or msgAddr.compareTo("*")==0) {
    cmd_buffer = cmd_buffer.substring(pos1);
    pos1 = cmd_buffer.indexOf('/',1);
    msgAddr = cmd_buffer.substring(1,pos1);
    if (msgAddr.compareTo("PING")==0) {
      client.publish("RGBASK",(char *)pongAns.c_str());
      cmd_buffer=="";
    } else {
...
void reconnect() {
  String connStr = "RGBClient";
  connStr += IPToStr(myIP);
  while (!client.connected()) {
    Serial1.print("Attempting MQTT connection...");
    if (client.connect((char *)connStr.c_str())) {
      ...
      Serial1.println(" try again in 1 second");
